...
async function kvGet(key: string) {
  const { data, error } = await kvSupabase
    .from(TABLE_NAME)
    .select('value')
    .eq('key', key)
    .single();
...
app.put('/make-server-12d56551/user/settings', async (c) => {
  try {
    ...
    await kvSet(`user:${user.id}`, updatedUser);
...
app.get('/make-server-12d56551/assistant', async (c) => {
  try {
    ...
    // Get first assistant
    const assistant = await kvGet(`assistant:${assistantIds[0]}`);
...
app.put('/make-server-12d56551/assistant', async (c) => {
  try {
    ...
    if (assistantIds.length === 0) {
      ...
      await kvSet(`assistant:${assistantId}`, assistant);
    ...
    const assistant = await kvGet(`assistant:${assistantId}`);
    ...
    await kvSet(`assistant:${assistantId}`, updatedAssistant);
...
app.get('/make-server-12d56551/assistants', async (c) => {
  try {
    ...
    const assistants = await Promise.all(
      assistantIds.map(id => kvGet(`assistant:${id}`))
    );
...
app.delete('/make-server-12d56551/assistants/:id', async (c) => {
  try {
    ...
    await kvDel(`assistant:${assistantId}`);
...
app.post('/make-server-12d56551/leads', async (c) => {
  try {
    ...
    await kvSet(`lead:${leadId}`, lead);

    // Update user's leads list
    const userData = await kvGet(`user:${assistant.userId}`);
    if (userData) {
      userData.leads = [...(userData.leads || []), leadId];
      await kvSet(`user:${assistant.userId}`, userData);
    }
...
app.put('/make-server-12d56551/leads/:id', async (c) => {
  try {
    ...
    const lead = await kvGet(`lead:${leadId}`);
    ...
    await kvSet(`lead:${leadId}`, updatedLead);
...
app.post('/make-server-12d56551/admin/init', async (c) => {
  try {
    ...
    const existingAdmin = await kvGet('user:admin');
    if (existingAdmin) {
    ...
    await kvSet('user:admin', data.user.id);
...
app.get('/make-server-12d56551/events', async (c) => {
  try {
    ...
    const events = await Promise.all(
      eventIds.map(id => kvGet(`event:${id}`))
    );
...
app.post('/make-server-12d56551/events', async (c) => {
  try {
    ...
    await kvSet(`event:${eventId}`, event);
...
app.put('/make-server-12d56551/events/:id', async (c) => {
  try {
    ...
    const event = await kvGet(`event:${eventId}`);
    ...
    await kvSet(`event:${eventId}`, updatedEvent);
...
app.post('/make-server-12d56551/feedback', async (c) => {
  try {
    ...
    if (!RESEND_API_KEY) {
      ...
      await kvSet(`feedback:${feedbackId}`, {
        id: feedbackId,
        userId: user.id,
        userEmail,
        userName,
